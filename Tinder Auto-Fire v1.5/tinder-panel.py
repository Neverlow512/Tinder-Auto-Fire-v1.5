...
def gen():
    ...
    .proxy-type-container select {{
        ...
        background-size: 10px 5px;
        cursor: pointer;
        width: 125%; /* Ensures the dropdown matches the width of the Number of Pictures field */
    }}
    .school-container {{
        position: relative;
        left: -8.4cm;
    ...
    <div class="form-row">
        ...
        <div class="killswitch-container">
            ...
            <div class="school-container">
                <label for="add_school" class="school-label">Add School:</label>
                <select id="add_school" name="add_school" onchange="toggleSchoolInput(); saveValue(this);">
                    <option value="off">Off</option>
                    <option value="on">On</option>
                </select>
            </div>
            <div id="school_input_container" class="school_input_container" style="display: none;">
                <br>Schools:<br>
                <textarea id='schools' name='schools' rows='10' cols='20' onkeyup='saveValue(this);'></textarea>
            </div>
        </div>
        <div class="pictures-dropdown-container">
            <label for="number_of_pictures">Number of Pictures:</label>
            <div class="custom-number-input">
                <input type="number" id="number_of_pictures" name="number_of_pictures" min="2" max="6" value="3" onchange='saveValue(this);'>
                <div class="number-input-buttons">
    ...
    function loadValues() {{
        ...
        document.getElementById("add_school").value = getSavedValue("add_school") || "off";
        document.getElementById("schools").value = getSavedValue("schools");
        toggleSchoolInput();
    }}
    ...
    function toggleSchoolInput() {{
        var schoolSelect = document.getElementById('add_school');
        var schoolInputContainer = document.getElementById('school_input_container');
        if (schoolSelect.value === 'on') {{
            schoolInputContainer.style.display = 'block';
        }} else {{
            schoolInputContainer.style.display = 'none';
        }}
    }}
    ...
    function startProcess() {{
        ...
        formData.append('add_school', document.getElementById('add_school').value);
        formData.append('schools', document.getElementById('schools').value);
...
def cmd():
    ...
    add_school = request.form.get('add_school', 'off')
        ...
        if x["device"] == device and x["status"] == 'ready':
            ...
            try:
                command = [
                    ...
                    profile_options_json, # Pass the JSON string as is
                    proxy_type,
                    add_school,
                    schools  
                ]
